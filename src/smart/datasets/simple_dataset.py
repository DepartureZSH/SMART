...
class BagDatasetPairAsUnit(Dataset):
    def __init__(self, data_dir, bag_data_file, split, tokenizer=None, txt_seq_len=70, img_seq_len=50,
    ...
    def get_image_item(self, item, pairs):
        # load data
        img_108076_id_str, object_classes, _, object_features, object_boxes = self.decode_features(item)
        assert len(object_features) == len(object_boxes)

        object_classes = object_classes[: self.img_seq_len]
        # object_caption = object_captions[: self.img_seq_len]
    ...
    def __getitem__(self, item):
        ...
        return (bag_key, bag_image_feats, None, bag_input_ids, bag_input_mask, bag_segment_ids, bag_object_boxes,
    ...
    def decode_features(self, item_idx):
        ...
        return img_108076_id_str, label_classes, None, object_features, label_boxes
...
if __name__ == '__main__':
    ...
    dataset = BagDatasetPairAsUnit(data_dir ,f'{data_dir}/{split}_bag_data.json', split, tokenizer, shuffle=False)
    for data in dataset:
        print(data)
        break