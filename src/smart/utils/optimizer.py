...
class WarmupReduceLROnPlateau(object):
    def __init__(
        ...
        if warmup_method not in ("constant", "linear", "cos"):
            raise ValueError(
                "Only 'constant' or 'linear' warmup_method accepted"
                "got {}".format(warmup_method)
            )
    ...
    def get_lr(self):
        warmup_factor = 1
        # during warming up
        adjusted_base_lrs = [base_lr * (self.gamma ** self.stage_count) for base_lr in self.base_lrs]
        if self.last_epoch < self.warmup_iters:
            if self.warmup_method == "cos":
                epoch = self.last_epoch + 1
                return [base_lr * epoch / self.warmup_iters for base_lr in self.base_lrs]
            if self.warmup_method == "constant":
                warmup_factor = self.warmup_factor
            elif self.warmup_method == "linear" or self.warmup_method == "cos":
                alpha = float(self.last_epoch) / self.warmup_iters
                warmup_factor = self.warmup_factor * (1 - alpha) + alpha
            ...
            # Cosine Annealing: Adjust the learning rate using cosine function
            epoch = self.last_epoch + 1
            decay_epochs = self.total_epochs - self.warmup_iters
            cosine_decay = 0.5 * (1 + math.cos(math.pi * (epoch - self.warmup_iters) / decay_epochs))
